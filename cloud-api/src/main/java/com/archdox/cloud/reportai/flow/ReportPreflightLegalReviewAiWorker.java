...
public class ReportPreflightLegalReviewAiWorker {
    ...
    public ReportPreflightLegalReviewAiWorker(Engine engine) {
        this.engine = engine;
    }

    public boolean submitAndAwait(AiHarnessFlow flow, Duration timeout) {
        engine.worker(ArchDoxRuntimeConfiguration.REPORT_PREFLIGHT_LEGAL_REVIEW_AI_WORKER)
    ...
    private void sleep() {
        try {
            ...
            throw new IllegalStateException("Interrupted while waiting for report legal review AI harness", ex);
        }
    }
}
