...
public class StandardTemplateFieldCatalog {
    ...
    private static final List<String> CURRENT_FOCUS_REPORT_TYPES = List.of(
    ...
    private static final List<String> CONSTRUCTION_REPORT_TYPES = List.of(
            "CONSTRUCTION_SUPERVISION_REPORT",
            "SUPERVISION_REPORT");
    private static final List<String> DAILY_REPORT_TYPES = List.of("CONSTRUCTION_DAILY_SUPERVISION_LOG");
    private static final List<String> ALL_DAILY_LOG_REPORT_TYPES = List.of("CONSTRUCTION_DAILY_SUPERVISION_LOG");
    ...
    private static final List<TemplateFieldDefinition> FIELDS = List.of(
            ...
            field("reportType", "Report type", "report", "report.reportType", "CONSTRUCTION_DAILY_SUPERVISION_LOG", "Normalized ArchDox report type."),
            ...
            field("workName", "Work name", "project-site", "project.name", "Document Tower", "Generic work name alias used by construction forms."),
            ...
            field("constructionTrade", "Construction trade", "daily-content", "steps.DAILY_LOG.payload.dailyItems.groups[0].tradeName", "Concrete", "Trade or work type for daily supervision.", DAILY_REPORT_TYPES),
            field("detailedProcess", "Detailed process", "daily-content", "steps.DAILY_LOG.payload.dailyItems.groups[0].processName", "Slab", "Detailed work process.", DAILY_REPORT_TYPES),
            field("floor", "Floor", "daily-content", "steps.DAILY_LOG.payload.dailyItems.groups[0].floor", "3F", "Floor or zone for the work item.", DAILY_REPORT_TYPES),
            field("dailyItems", "Daily supervision item groups", "daily-content", "steps.DAILY_LOG.payload.dailyItems", "Trade groups with supervision entries", "Structured construction daily log rows grouped by trade, process/floor, inspection item, supervision content, and photo ids.", DAILY_REPORT_TYPES),
            ...
            field("inspectionItem", "Inspection item", "daily-content", "steps.DAILY_LOG.payload.dailyItems.groups[0].entries[0].inspectionItemName", "Rebar count and pitch", "Catalog-managed inspection item from the construction supervision checklist.", DAILY_REPORT_TYPES),
            field("supervisionItem", "Supervision item", "daily-content", "steps.DAILY_LOG.payload.dailyItems.groups[0].entries[0].inspectionItemName", "Rebar spacing", "Compatibility label for the canonical inspection item.", DAILY_REPORT_TYPES),
            field("supervisionContent", "Supervision content", "daily-content", "steps.DAILY_LOG.payload.dailyItems.groups[0].entries[0].checklistRows", "Checked rebar", "Generated from structured daily checklist rows.", ALL_DAILY_LOG_REPORT_TYPES),
            ...
            field("nextAction", "Next action", "daily-content", "steps.REMARKS.payload.nextAction", "Recheck requested", "Follow-up action or reinspection plan for daily logs.", ALL_DAILY_LOG_REPORT_TYPES),
            field("correctiveAction", "Corrective action", "checklist-construction", "steps.*.payload.correctiveAction", "Corrected", "Corrective action for construction supervision issue handling."),
            ...
            field("comprehensiveOpinion", "Comprehensive opinion", "report-opinion", "steps.REMARKS.payload.comprehensiveOpinion", "Suitable", "Overall opinion for construction supervision reports.", CONSTRUCTION_REPORT_TYPES),
            field("checklistSummary", "Checklist summary", "checklist-construction", "steps.CHECKLIST.payload.checklistSummary", "Checked", "Summary of checklist answers."),
            field("issueCount", "Issue count", "checklist-construction", "steps.CHECKLIST.payload.issueCount", "0", "Number of checklist or inspection issues."),
            field("checklistPhotoSummary", "Checklist photo summary", "checklist-construction", "checklistPhotos", "Field investigation / Photos: 2", "Compact text list of checklist-linked photos.")
    );
    ...
    private static final List<TemplateFormPreset> PRESETS = List.of(
            ...
            officialPreset(
                    ...
                    "공식 제출용 공사감리일지 별지 2",
                    "ArchDox의 공사감리일지 표준입니다. 별지 제2호서식 기준의 공식 제출용 렌더러를 사용합니다.",
                    RENDERING_BUNDLED_OFFICIAL_RENDERER,
                    DAILY_REPORT_TYPES,
                    List.of("documentTitle", "serialNo", "chiefSupervisorName", "architectAssistantName", "assistantArchitectName", "constructionName", "inspectionDate", "inspectionDayOfWeek", "weather", "dailyItems", "specialNotes", "issueAndAction", "correctionResults", "nextAction"),
                    List.of("CHECKLIST_TABLE", "CHECKLIST_PHOTO_TABLE", "PHOTO_TABLE")),
            officeInternalPreset(
                    "OFFICE_INTERNAL_CONSTRUCTION_SUPERVISION_REPORT",
                    "사무소 내부용 감리보고서",
                    "사무소별 보고 문구, 사진 배치, 내부 검토 섹션을 조정할 수 있는 감리보고서 프리셋입니다.",
                    CONSTRUCTION_REPORT_TYPES,
    ...
    public TemplateFieldCatalog catalog(String reportType) {
        ...
        if (normalizedReportType != null && !CURRENT_FOCUS_REPORT_TYPES.contains(normalizedReportType)) {
            return new TemplateFieldCatalog(normalizedReportType, List.of(), List.of());
        }
    ...
    private boolean supports(List<String> reportTypes, String reportType) {
        if (reportType == null) {
            return reportTypes.isEmpty()
                    || reportTypes.stream().anyMatch(CURRENT_FOCUS_REPORT_TYPES::contains);
        }
        return reportTypes.isEmpty() || reportTypes.contains(reportType);
    }
