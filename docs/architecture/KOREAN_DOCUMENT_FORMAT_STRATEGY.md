...
## Official vs Office Internal Templates

ArchDox must support both public-form output and office-operational output.
Those are not separate data models. They are different template/layout choices
over the same report snapshot.

Template presets and bundled template catalog entries should expose:

- `templateKind`
  - `OFFICIAL_SUBMISSION`: a bundled or copied form intended to stay close to a
    public-agency reference form.
  - `OFFICE_INTERNAL`: an office-facing form for internal review, client
    sharing, meeting notes, quality tracking, or custom operations.
- `customizationPolicy`
  - `COPY_AND_OVERRIDE`: use the system default as a seed, then publish an
    office-specific revision when wording/layout differs.
  - `OFFICE_EDITABLE`: a preset intended for office-owned custom templates.
- `renderingPolicy`
  - `DOCX_TEMPLATE_BINDING`: normal placeholder and rich-section binding.
  - `BUNDLED_OFFICIAL_RENDERER`: a narrow temporary renderer used only for a
    specific bundled official template when matching a public form is more
    important than free layout changes.

The construction daily supervision log currently uses
`BUNDLED_OFFICIAL_RENDERER` only for the bundled default `storageRef`. When an
office uploads its own template revision, the engine must respect that custom
template and use the normal DOCX binding path.

This keeps the customer story simple:

```text
public submission form
  -> use or copy the official bundled template

office internal form
  -> upload/publish an office template revision with the same field catalog
```
