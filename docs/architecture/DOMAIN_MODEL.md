...
- Worker types:
  - `ARCHDOX_AGENT`: generated by the selected ArchDox Agent through the
...
- Rule: Cloud API must not execute `document-engine` directly and must not add an
  inline render fallback. The selected `archdox-agent` advertises output
  capabilities and owns export runtime dependencies such as LibreOffice.
...
- Rule: office plans should prefer an authenticated `LOCAL_OFFICE` agent when it
  is installed and online. Personal plans, and offices without local runtime,
  should use a `CLOUD_MANAGED` ArchDox Agent.
- Rule: `DocumentRenderFlow` routes to `ARCHDOX_AGENT`; deployment mode and
  capabilities decide the concrete target. UI polling must not care whether
  rendering was done by a `LOCAL_OFFICE` or `CLOUD_MANAGED` Agent.
