...
public final class NarrativePolishHarnessFactory {
    ...
    public static final PromptVersion PROMPT_VERSION = new PromptVersion("archdox-document-narrative-polish", "1.0.2");

    private final ObjectMapper objectMapper;

    public NarrativePolishHarnessFactory(ObjectMapper objectMapper) {
        this.objectMapper = Objects.requireNonNull(objectMapper, "objectMapper must not be null");
    }

    public AiHarnessSpec<NarrativePolishInput, NarrativePolishResult> spec(FindingSink findingSink) {
        return spec(findingSink, AiHarnessRunStore.noop(), new MaxAttemptsRefinePolicy(2));
    }

    public AiHarnessSpec<NarrativePolishInput, NarrativePolishResult> spec(
        ...
        var builder = AiHarnessSpec.<NarrativePolishInput, NarrativePolishResult>builder()
                .harnessId(HARNESS_ID)
                .defaultModelId(DEFAULT_MODEL_ID)
                .defaultTimeout(Duration.ofSeconds(90))
                .promptVersion(PROMPT_VERSION)
                .promptBuilder(new NarrativePolishPromptBuilder(objectMapper))
                .validator(new JacksonPojoSchemaValidator<>(NarrativePolishResult.class, objectMapper))
                .refinePolicy(refinePolicy)
                .runStore(runStore)
                .findingExtractor(new NarrativePolishFindingExtractor())
                .findingSink(findingSink);
